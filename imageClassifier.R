#For the first time,in console windows,enter:
...
poster_png <- list.files(pattern="*.png")
#poster_jpg_list, poster_png_list store matrixs of jpg and png images.
poster_jpg_list <- list()
poster_png_list <- list()

bad_poster_jpg <- list()
bad_poster_png <- list()
bad_original_jpg <- list()
bad_original_png <- list()

#grayresize read in all images in a file, grayscale and resize to be 96*96, then store
#the matrix into a list
GrayResize <- function (imgList,n) {
  matList <- list()
  bad_list <- list()
  ...
  for(i in 1:imgNum) {
    ...
    matList[[i]] <- gray_Mat
      ...
      bad_list[count]<-imgList[i]
      count <- count+1
    }
  }
  if(n == 1){
    write.xlsx(bad_list, "/home/visionexpts/bad1.xlsx")
  }
  if(n == 2){
    write.xlsx(bad_list, "/home/visionexpts/bad2.xlsx")
  }
  if(n == 3){
    write.xlsx(bad_list, "/home/visionexpts/bad3.xlsx")
  }
  if(n == 4){
    write.xlsx(bad_list, "/home/visionexpts/bad4.xlsx")
  }
  return(matList)
}

poster_jpg_list <- GrayResize(poster_jpg,1)
poster_png_list <- GrayResize(poster_png,2)

read_poster_time <- proc.time() - ptm
cat("Reading posters cost:", read_poster_time,"\n")
#######################################################
##            Read in all the originals        2min  ##
#######################################################
...
origin_png <- list.files(pattern = "*.png")
#origin_jpg_list, origin_png_list store matrixs of jpg and png images.
origin_jpg_list <- list()
origin_png_list <- list()

origin_jpg_list <- GrayResize(origin_jpg,3)
origin_png_list <- GrayResize(origin_png,4)
#poster_list stores all posters(jpg + png), origin_list stores all originals(jpg+png)
poster_list <- c(poster_jpg_list,poster_png_list)
origin_list <- c(origin_jpg_list,origin_png_list)

read_original_time <- proc.time() - read_poster_time
cat("Reading posters cost:", read_original_time,"\n")
################################################################
...
poster_test_index <- sample(1:poster_num, round(0.1*poster_num))
origin_test_index <- sample(1:origin_num, round(0.1*origin_num))
...
patch_extraction_time <- proc.time() - read_original_time
cat("Patch extraction cost:", patch_extraction_time,"\n")
...
kmean_time <- proc.time() - patch_extraction_time
cat("K mean cost:", kmean_time,"\n")
...
feature_Generator <- function(inputList,centroids) {
  ...
  count<-0
  for(i in 1:num) {
    pMat <- matsplitter(inputList[[i]],unit,s)
...
trans_time <- proc.time() - kmean_time
cat("trans cost:", kmean_time,"\n")
###################################################################################
...
for(i in 1:fold){
  ...
  model <- ksvm(train_x,train_y,type="C-svc",kernel='vanilladot',C=30,scaled=c())
...
val_time <- proc.time() - trans_time
cat("val cost:", val_time,"\n")
...
cat("testing:K=800,unit=16,s=2,accuracy=",test_accuracy)


test_time <- proc.time() - val_time
cat("test cost:", test_time,"\n")
